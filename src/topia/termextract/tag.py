...
import os
import re
import re2        # You must use re2 or you might get killed by exponential regex evaluation.
...
TERM_SPEC2 = re2.compile('([\W\d_]*)(([^\W\d_]*[-\.]*)*[^\W\d_])([\W\d_]*[^\W\d_]*)', re.UNICODE)
...
class Tagger(object):
    ...
    def tokenize(self, text):
        ...
        for term in re.split('\s', text):
            ...
            # split those out
#            orig_match = ORIG_TERM_SPEC.search(term)
            @timeout(1)
            def slow_match(term):
...
#            match = TERM_SPEC.search(term)
            ...
            if match2 is None:
                import sys
                print >> sys.stderr, "WHA?!? %s / re MATCHES / re2 DOESNT" % (term.encode("utf-8")), repr(term)
                continue
            if match.groups() != match2.groups():
#                om = orig_match.groups()
                m = match.groups()[0], match.groups()[1], match.groups()[3]
                m2 = match2.groups()[0], match2.groups()[1], match2.groups()[3]
                import sys
#                if om != m or m2 != m:
#                    print >> sys.stderr, "WHA?!? %s / origre = %s / re = %s / re2 = %s" % (term, om, m, m2)
                if m2 != m:
                    print >> sys.stderr, "WHA?!? %s / re = %s / re2 = %s" % (term, m, m2), repr(term)
